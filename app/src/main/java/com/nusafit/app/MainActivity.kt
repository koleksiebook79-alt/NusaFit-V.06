...
class MainActivity:AppCompatActivity(),OnMapReadyCallback{
                ...
                DialogInterface.OnClickListener { _, _ ->
                    Store.saveProfile(
                        ...
                        Profile(
                            ...
                            age.text.toString().toIntOrNull() ?: 0,
                            height.text.toString().toDoubleOrNull() ?: 0.0,
            ...
            .setPositiveButton(
                ...
                DialogInterface.OnClickListener { _, _ ->
                    ...
                    val intent = Intent(this, ReminderReceiver::class.java)
                        ...
                        .putExtra(
                            "name",
                            name.text.toString().ifBlank { "Olahraga" }
                        )
    ...
    private fun statsDialog() {
        val activities = Store.activities(this)
        val now = System.currentTimeMillis()

        val week = activities.filter {
            now - it.start <= 7 * 86400000L
        }
        val month = activities.filter {
            now - it.start <= 30 * 86400000L
        }
        val year = activities.filter {
            now - it.start <= 365 * 86400000L
        }

        fun summary(records: List<ActivityRecord>): String {
            val distance = records.sumOf { it.distanceKm }
            val calories = records.sumOf { it.calories }.toInt()
            return "${records.size} sesi • ${
        ...
        val box = LinearLayout(this).apply {
            ...
            addView(StatsChartView(this@MainActivity, activities))
        }

        val summaryText = TextView(this).apply {
            text = "7 hari: ${summary(week)}\n" +
                "30 hari: ${summary(month)}\n" +
                "365 hari: ${summary(year)}"
            setPadding(0, 18, 0, 0)
        }
        box.addView(summaryText)

        AlertDialog.Builder(this)
            .setTitle("Statistik & Capaian")
            .setView(box)
